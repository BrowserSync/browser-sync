...
module.exports.callbacks = {
    ...
    "service:running": function (options, data) {
        ...
        if (type === "server") {
            ...
            var baseDir = options.getIn(["server", "baseDir"]);

            logUrls(options.get("urls"));
        ...
        if (type === "proxy") {
            ...
            logger.info("Proxying: {cyan:%s}", options.getIn(["proxy", "target"]));
            logger.info("Now you can access your site through the following addresses:");
            logUrls(options.get("urls"));
        }

        if (type === "snippet" && options.get("logSnippet")) {
