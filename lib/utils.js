...
var utils = {
    ...
    getUrlOptions: function(options) {
        ...
        return Immutable.fromJS(
            utils.getUrls(external, localhost, scheme, options)
        );
    },
    /**
    ...
    getUrls: function(external, local, scheme, options) {
        var urls = {
            local: utils.getUrl(
                utils._makeUrl(scheme, local, options.get("port")),
                options
            )
        };

        if (external !== local) {
            urls.external = utils.getUrl(
                utils._makeUrl(scheme, external, options.get("port")),
                options
            );
        }
    ...
    openBrowser: function(url, options, bs) {
        ...
        else if(_.isObject(open)){
            commonUrl = open.get("url");
            url = commonUrl.slice(0, 1) === "/" ? path.join(url , commonUrl) : commonUrl;
        }
