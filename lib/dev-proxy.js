...
module.exports = function (host, ports, options) {
    ...
    var server = httpProxy.createServer(function (req, res, proxy) {
        ...
        tags = messages.scriptTags(host, ports[0], ports[1]);
    ...
    server.proxy.on("proxyResponse", function (req, res, response) {
        var cLength;
        if (utils.isExcluded(req)) return;
        if (response.headers.hasOwnProperty("content-length")) {
            cLength = parseInt(response.headers['content-length'], 10);
            cLength += tags.length;
            response.headers['content-length'] = cLength.toString();
        }
    });
