...
module.exports = {
    ...
    startSockets: function (bs, done) {
        ...
        var clientEvents = bs.pluginManager.hook("client:events", bs.options.get("clientEvents").toJS()); // todo - Immutable?
    ...
    startUi: function (bs, done) {
        ...
        var ui          = bs.pluginManager.get("UI");
        var uiOpts;

        // if user provided a UI, use it instead
        if (userPlugins.some(function (item) {
            return item.name === "UI";
        })) { // do nothing if user-provided plugin
            delete bs.pluginManager.defaultPlugins["UI"];
            return done();
        }

        if (!(uiOpts = bs.options.get("ui"))) {
            return done();
        }

        var opts      = uiOpts.toJS();
        opts.logLevel = bs.options.get("logLevel");

        if (ui) {

            ui = ui(opts, bs);

            return ui.getServer(function (err, server) {
    ...
    mergeUiSettings: function (bs, done) {
        ...
        if (!bs.ui) {
            ...
            options: {
                urls: bs.options.get("urls").merge(bs.ui.instance.options.get("urls"))
            }
        });
    },
    /**
