...
module.exports.init = function (userFiles, userConfig, cb) {
    var config = exports.mergeConfigs(userFiles, userConfig);
    return exports.start(config.files, config.config, cb);
};

/**
 * @param userFiles
 * @param userConfig
 * @returns {{files: Array, config: object}}
 */
module.exports.mergeConfigs = function (userFiles, userConfig) {
    var defaultsClone = _.cloneDeep(defaultConfig);
    var config = _.merge(defaultsClone, userConfig, function (a, b) {
        return _.isArray(a) ? _.union(a, b) : undefined;
    });

    var files  = options._mergeFilesOption(userFiles, config.exclude);
    config = init.mergeOptions(defaultsClone, config, init.allowedOptions);
