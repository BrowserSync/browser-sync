...
(function (window, socket) {
    ...
    var ghost = {
        ...
        getScrollPercentage: function () {
            var scrollSpaceX, scrollSpaceY, scroll, psy, psx, d = document,
                r = d.documentElement, b = d.body;

            scrollSpaceX = b.offsetWidth - r.clientWidth;
            scrollSpaceY = b.scrollHeight - r.clientHeight;

            scroll = this.getScroll();
            psx = scroll[0] / scrollSpaceX;
            psy = scroll[1] / scrollSpaceY;
        ...
        getInputs: function () {
            ...
            var inputTypes = ["text", "email", "url", "tel"];
    ...
    socket.on("scroll:update", function (data) {
        if (data.url === window.location.host + window.location.pathname) {
            var scrollSpaceY, d = document, r = d.documentElement, b = d.body;
            scrollSpaceY = b.scrollHeight - r.clientHeight;
            scope.ghostMode.enabled = false;
            window.scrollTo(0, scrollSpaceY * data.position); // proportional scroll
        }
    });
