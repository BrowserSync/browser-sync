...
browserSync.prototype = {
    ...
    startServices: function (args) {
        ...
        // launch the server
        this.initServer(this.getHostIp(options, devIp.getIp(null)), ports, options);
    },
    /**
     *
     ...
     * @returns {*|http.Server}
     */
    initServer: function (host, ports, options) {

        var proxy  = options.proxy || false;
        var server = options.server || false;
        var msg;

        var servers = serverModule.launchServer(host, ports, options);
     ...
     */
    watchFiles: function (watcher, watchCallback, changeCallback) {
