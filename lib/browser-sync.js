 ...
 * @returns {*|http.Server}
 */
BrowserSync.prototype.initServer = function (host, ports, options, scripts) {
    ...
    var servers = serverModule.launchServer(host, ports, options, scripts);
    ...
    if (type && (server || proxy)) {
        if (options.tunnel) {
            ...
            localtunnel(ports[type], function(err, tunnel) {
                if (err) {}
                emitEvent(tunnel.url);
            });
        } else {
            emitEvent(options.url);
        }
    }

    function emitEvent(url) {
        events.emit("running", {
            type: type,
