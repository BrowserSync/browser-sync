...
browserSync.prototype = {
    ...
    startServices: function (args) {
        ...
        // launch the server
        if (options.server || options.proxy) {
            this.initServer(options.host, ports, options, ioLocal);
        } else {
            msg = messages.init(options.host, ports);
            this.log(msg, options, true);
        }

        controlPanel.launchControlPanel(options.host, ports, options, ioLocal);
    },
    /**
     * @returns {*}
     */
    ...
    initServer: function (host, ports, options, ioLocal) {
        ...
        if (open) {
            // Add test for this!
            options.url = messages._makeUrl(host, ports[open]);
