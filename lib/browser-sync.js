 ...
 * @param {Function} [cb]
 */
BrowserSync.prototype.registerPlugin = function (name, module, cb) {
    if ("function" !== typeof module.plugin) {
        return typeof cb === "function" ? cb("Module must have a .plugin() method") : false;
    }
    return this.plugins["plugin:" + name] = module; // every plugin should be a callable function
};
...
BrowserSync.prototype.getHook = function (name, hook) {
    if (_.isFunction(this.plugins[name][hook])) {
        return this.plugins[name][hook](this);
    }
};
...
BrowserSync.prototype.registerInternalEvents = function (options) {
    ...
    var events = {
        ...
        "msg:info": function (data) {
            ...
            infoLog(data.msg, data.vars || "", typeof data.prefix === "undefined" ? true : data.prefix);
        },
        "init": function () {
            this.active = true;
        }
...
BrowserSync.prototype.cleanup = function () {
    ...
    utils.log("info", messages.exit(), this.options);
...
BrowserSync.prototype.initServer = function (options, scripts) {
    ...
    var bsServer = serverModule.launchServer(options, scripts);
    ...
    options.url = utils.getUrl(utils._makeUrl(options.host, options.port), options);
