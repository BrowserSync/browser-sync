...
BrowserSync.prototype.init = function (options, cb) {
    ...
    this.pluginManager.defaultPlugins["tunnel:runner"] = {plugin: function () {/*noop*/}};

    this.logger   = this.pluginManager.get("logger")(this.events, this);
    ...
    if (options.get("server") && options.get("proxy")) {
        ...
        err = "Invalid config. You cannot specify both server & proxy options.";
 ...
 * @param {Number} port
 */
BrowserSync.prototype.handleSuccess = function (port) {

    var debug = this.debug;

    debug("Using port {magenta:%s", port);

    this.setOption("port", port);
    this.setOption("scheme", utils.getScheme(this.options));

    var init = function () {
        services.init(this)(this.options);
    }.bind(this);

    if (_.isUndefined(this.options.get("online")) && _.isUndefined(process.env.TESTING)) {
        ...
        "file:changed": function (data) {
            ...
            if (!this.paused && data.namespace === "core") {
                if (_.isUndefined(data.log)) {
                    data.log = this.options.get("logFileChanges");
                }
