...
var scriptData = fs.readFileSync(__dirname + "/control-panel" + messages.urls.clientScript, "UTF-8");
var controlPanelData = fs.readFileSync(__dirname + "/control-panel" + messages.urls.controlPanelScript, "UTF-8");
...
browserSync.prototype = {
    ...
    init: function (files, options) {
        ...
        var _this = this, io, handles, server, watcher, controlPanel, host;

        host = _this.getHostIp(options);

        this.getPorts(4, function (ports) {
            ...
            controlPanel = _this.launchControlPanel(host, ports);

            // launch the server
            server = _this.launchServer(host, ports, options);
    ...
    handleSocketConnection: function (events, userOptions, handle) {
        ...
        io.sockets.on("connection", function (client) {
            ...
            // Control panel
            if (client.handshake.headers.referer === "http://0.0.0.0:3003/") {

                controlPanelClient = client;
                controlPanelClient.emit("controlPanel:init", {
                    options: userOptions
                });
            } else {
                client.emit("connection", userOptions);
                ua = client.handshake.headers['user-agent'];
                if (controlPanelClient) {
                    controlPanelClient.emit("controlPanel:connection", {browser: parser.setUA(ua).getBrowser()});
                }
     ...
     * @returns {http.Server}
     */
    launchControlPanel: function (host, ports) {

        var modifySnippet = function (req, res) {
            res.setHeader("Content-Type", "text/javascript");
            res.end(messages.socketConnector(host, ports[0]) + controlPanelData);
        };

        var modifySnippetClient = function (req, res) {
            res.setHeader("Content-Type", "text/javascript");
            res.end(messages.socketConnector(host, ports[0]) + scriptData);
        };

        var app = connect()
                .use(loadSnippet(host, ports[0], ports[3], "control-panel"))
                .use(messages.urls.controlPanelScript, modifySnippet)
                .use(messages.urls.clientScript, modifySnippetClient)
                .use(connect.static(filePath.resolve(__dirname + "/control-panel")));

        var server = http.createServer(app).listen(ports[3]);
        return server;
    },
    /**
    ...
    launchServer: function (host, ports, options) {
        ...
        if (proxy) {
            ...
            app = connect()
                    .use(loadSnippet(host, ports[0], ports[3]))
        ...
        if (!options.server && !proxy) {
            msg = messages.init(host, ports[0], ports[3]);
        }
