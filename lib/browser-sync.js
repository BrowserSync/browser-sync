...
browserSync.prototype = {
    ...
    init: function (files, options) {
        ...
        var _this = this, minPorts = this.options.minPorts, startServices = this.startServices;
        var portRange = this.getPortRange(minPorts, options);

        if (portRange) {
            this.getPorts(minPorts, function (ports) {
                startServices.apply(_this, [ports, files, options]);
    ...
    startServices: function () {
        ...
        var iolocal, handles, server, watcher;

        // setup Socket.io
        iolocal = this.setupSocket(ports);

        // Set up event callbacks
        handles = this.handleSocketConnection(this.callbacks, options, this.handleClientSocketEvent);
        // launch the server

        server = this.launchServer(this.getHostIp(options, devIp.getIp(null)), ports, options);

        // Watch files
        watcher = this.watchFiles(files, iolocal, this.changeFile, options);
    },
    /**
