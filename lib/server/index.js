...
module.exports.plugin = function (bs, scripts) {
    ...
    var debug   = bs.debug;
    var proxy   = options.get("proxy")   || false;
    var server  = options.get("server")  || false;
    var serverObj;
    ...
    if (server || snippet) {
        debug("Static Server running ({magenta:%s}) ...", options.get("scheme"));
    ...
    if (bsServer) {
        bs.server = bsServer.listen(options.get("port"));
    }

    debug("Running mode: %s", type.toUpperCase());

    if (type && (server || proxy)) {
        if (options.get("tunnel") && options.get("online")) {
            try {
            ...
            finished(options.getIn(["urls", "local"]));
        }
    } else {
        finished("n/a");
    }
    ...
    function finished (url, tunnel) {
        ...
        if (tunnel) {
            bs.options = bs.options.updateIn(["urls"], function (map) {
                return map.set("tunnel", url);
            });
        }
