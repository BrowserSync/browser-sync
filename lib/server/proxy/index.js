...
function init(opts, proxy, additionalRules, additionalMiddleware, errHandler) {
    ...
    function unzipRequest(req, res, next) {
        ...
        res.write = function (data, encoding) {
            buffers.push(data);
            if (res._headers['content-type'] !== 'text/html' || !(res._headers && res._headers['content-encoding'] && (res._headers['content-encoding'] === 'gzip' || res._headers['content-encoding'] === 'deflate'))) {
            ...
            if (buffer.length > 0) {
                if (res._headers['content-type'] === 'text/html' && res._headers && res._headers['content-encoding'] && (res._headers['content-encoding'] === 'gzip' || res._headers['content-encoding'] === 'deflate')) {
                  zlib.unzip(buffer, function(err, buffer) {
                    delete cached[req.url].headers['content-encoding'];
                    delete cached[req.url].headers['accept-ranges'];
                    delete res._headers['content-encoding'];
                    delete res._headers['accept-ranges'];
