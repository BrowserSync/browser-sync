...
var utils = {
    ...
    modifySnippet: function (host, port) {
        ...
        var jsFile    = fs.readFileSync(__dirname + messages.clientScript);
 ...
 */
module.exports.launchServer = function (host, ports, options) {

    var proxy  = options.proxy || false;
    var server = options.server || false;
    var app, modifySnippet = utils.modifySnippet(host, ports[0]);
    ...
    if (proxy) {
        proxyServer = proxyModule.createProxy(host, ports, options);
    } else {
