...
module.exports = {
     ...
     * @returns {String}
     */
    scriptTags: function (hostIp, ports, options) {

        var template = "<script src='{: url1 :}'></script>\n<script src='{: url2 :}'></script>\n\n";
        var params = {
            url1: this._makeUrl(hostIp, ports.socket) + this.socketIoScript,
            url2: this._makeUrl(hostIp, ports.controlPanel) + this.clientScript(options)
        };
