...
module.exports = {
    ...
    init: function (hostIp, socketIoPort, scriptPort) {
        ...
        var template = "\n\n{cyan:BS: All Set Up!} Now copy & paste this snippet just before the closing </body> tag in your website.\n\n <%= tags %>";
        var params = {
    ...
    initServer: function (hostIp, scriptPort, baseDir) {
        ...
        var template = "\n{green:BS: OK, Server running at} {magenta:<%= url %>}";
        template    += "\n{green:BS: Serving files from: } {magenta:<%= baseDir %>}";
        template    += "\n\n{green:BS: Load a browser & check back here. If you set up everything correctly, you'll see a} {cyan:'Browser Connected'} {green: message}\n";
        var params = {
    ...
    fileWatching: function (patterns) {
        ...
        if (Array.isArray(patterns) && patterns.length) {
            ...
            string = compile("{green:BS: Watching the following:}\n");
            ...
            string = compile("{red:BS: Not watching any files...}");
        }
    ...
    browser: {
        reload: function () {
            var string = "{cyan:BS: Reloading all connected browsers...}";
            return compile(string);
        },
        inject: function () {
            var string = "{cyan:BS: Injecting file into all connected browsers...}";
            return compile(string);
        }
    },
