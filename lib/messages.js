...
module.exports = {
     ...
     * @returns {String}
     */
    initServer: function (options, port, baseDir, tunnel) {
        ...
        var templates = [
            ...
            "{green:Local:}    >>> {magenta:{:local:}}",
        ];

        if (options.online) {
            templates.push("{green:External:} >>> {magenta:{:remote:}}\n");
        }

        var params = {
            local: this._makeUrl("localhost", port, "http:"),
            remote: this._makeUrl(options.host, port, "http:")
        };
        ...
        if (tunnel) {
            var tunnelTemplates = [
                "{green:Tunnel:}   >>> {magenta:{:tunnel:}}\n\n"
            ];
            output += prefixed(tunnelTemplates, {tunnel: tunnel});
        }
     ...
     * @returns {string}
     */
    initProxy: function (options, proxyUrl, tunnel) {

        var output   = "";
        var local    = options.urls.local;
        var external = options.urls.remote;
    ...
    socketConnector: function (port, options) {
        ...
        var tunnel = "var ___socket___ = io.connect('http://' + location.hostname);";
