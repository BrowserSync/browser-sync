"use strict";

var utils        = require("./utils").utils;

/**
 * @param context
 * @returns {function(this:*)}
 */
module.exports.init = function (context) {

    /**
     * @this {BrowserSync}
     */
    return function (port, files, options) {

        this.options      = options;
        this.options.port = port;

        // register internal events
        this.registerInternalEvents(options);

        // Set global URL options
        options.urls = utils.setUrlOptions(port, options);

        files = this.pluginManager.hook("files:watch", this, files);

        // Start file watcher
        this.watchers = this.pluginManager.get("file:watcher")(files, options, this.events);

        // Get the Client JS + any user-defined
        var clientJs = this.clientJs = this.pluginManager.hook("client:js", {port: port, options: options});

        // Start the server
        var server = this.server = this.initServer(
            options,
            this.pluginManager.get("client:script")(options, clientJs, options.proxy ? "file" : "middleware")
        );

        // Start the socket, needs an existing server.
        this.io = this.pluginManager.get("socket")(server, this);

        this.events.emit("init", this);

        this.pluginManager.initUserPlugins(this);

        this.callback(null, this);

    }.bind(context);
};