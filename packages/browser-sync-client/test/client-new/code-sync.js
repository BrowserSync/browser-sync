describe("Code Sync", function() {
    ...
    describe("reload() with css files", function() {
        ...
        it("can call reload", function() {
            ...
            var elemsStub = sinon
                .stub(codeSync, "getElems")
                .returns({ elems: matches, attr: "href" });
        ...
        it("swaps url when css file has existing params", function() {
            ...
            assert.equal(
                actual.elem.href,
                "http://localhost.com/css/styles.css?name=kittie&id=||&rel=100",
                "Should append rel attribute"
            );
            matchesStub.restore();
            elemsStub.restore();
            clock.restore();
        ...
        it("swaps url when css file has no params", function() {
            ...
            assert.equal(
                actual.elem.href,
                "http://localhost.com/css/styles.css?rel=100",
                "Should append rel attribute"
            );
            matchesStub.restore();
            elemsStub.restore();
    ...
    describe("Saving scroll position", function() {
        it("should save scroll position in window.name", function() {
            ...
            assert.equal(
                $window.name,
                '<<BS_START>>{"bs":{"hardReload":true,"scroll":{"x":0,"y":0}}}<<BS_END>>'
            );
        ...
        it("should save scroll position in window.name", function() {
            ...
            assert.equal(
                $window.name,
                'NG_SOME_VALUE<<BS_START>>{"bs":{"hardReload":true,"scroll":{"x":0,"y":0}}}<<BS_END>>'
            );
            window.__bs_utils__.getWindow.restore();
        });
        it("should save scroll position in window.name", function() {
            ...
            assert.equal(
                $window.name,
                'NG_SOME_VALUE<<BS_START>>{"bs":{"hardReload":true,"scroll":{"x":0,"y":100}}}<<BS_END>>'
            );
            codeSync.saveScrollInName();
