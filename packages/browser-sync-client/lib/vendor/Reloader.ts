...
export function reload(document: Document, navigator: Navigator) {
    ...
    function reloadImages(path, document): Observable<any> {
        ...
        return merge(
            from([].slice.call(document.images)).pipe(
    ...
    function reloadStyleImages(style, styleNames: string[], path, expando): Observable<any> {
        return from(styleNames).pipe(
            ...
            map(([style, styleName, value, newValue, pathName]) =>
                styleSet({ style, styleName, value, newValue, pathName })
            )
        );
    }
    ...
    function reattachStylesheetLink(
        ...
        return Observable.create(obs => {
            ...
            mergeMap(() => {
                return timer(additionalWaitingTime).pipe(
                    tap(() => {
    ...
    function generateCacheBustUrl(url, expando = generateUniqueString(Date.now())) {
        ...
        let params = oldParams.replace(
            /(\?|&)browsersync=(\d+)/,
            (match, sep) => `${sep}${expando}`
        );
        if (params === oldParams) {
    ...
    function reloadStylesheet(path: string, document: Document, navigator): Observable<any> {
        ...
        const links: HTMLLinkElement[] = array(document.getElementsByTagName("link")).filter(
            link => {
         ...
         * @type {any[]}
         */
        const linksImported = links.reduce((acc, link) => {
        ...
        if (match) {
            ...
            if (links.length) {
                ...
                if (first === "*") {
                    return from(
                        links.map(link => reattachStylesheetLink(link, document, navigator))
                    ).pipe(mergeAll());
                }
            }
        }
