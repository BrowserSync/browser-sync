    ...
    return (
        require("immutable")
            ...
            .map(function(plugin) {
                ...
                try {
                    pkg = require("immutable").fromJS(
                        require(path.join(moduleName, "package.json"))
                    );
                } catch (e) {
                    console.error(e);
                    return plugin;
            ...
            .map(function(plugin) {
                ...
                if (markup) {
                    plugin = plugin.set(
                        "markup",
                        fs.readFileSync(path.resolve(plugin.get("relpath"), markup), "utf8")
                    );
                }
            ...
            .map(function(plugin) {
                ...
                return resolveIfPluginHas(
                    ["pkg", "browser-sync:ui", "hooks", "templates"],
                    "templates",
                    plugin
                );
            })
            ...
            .map(function(plugin) {
                ...
                return resolveIfPluginHas(
                    ["pkg", "browser-sync:ui", "hooks", "client:js"],
                    "client:js",
                    plugin
                );
            })
...
function resolvePluginFiles(collection, relPath) {
    return Immutable.fromJS(
        collection.reduce(function(all, item) {
